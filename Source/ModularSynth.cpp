...
// https://www.desmos.com/calculator/qtpxfura82
void ModularSynth::ZoomView(float zoomAmount, bool fromMouse)
{
   ...
   // canvas space point representing the center of the zoom with the old scale
   ...
   if (fromMouse)
   {
      ...
      // -> scaled (/ oldDrawScale)
      // -> center point (* 0.5f)
      // -> convert point to canvas space: convert the DrawOffset to a canvas position and add it to the
